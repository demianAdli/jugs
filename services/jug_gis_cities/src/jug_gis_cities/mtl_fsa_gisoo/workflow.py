...
def run_workflow(fsa):
  ...
  try:
    ...
    with _workflow_step('join nrcan with inter summary', normalized_fsa):
      ...
      nrcan_joined_summary = ScrubLayer(
        paths.qgis_path,
        output_paths['nrcan_joined_summary'],
        f'nrcan_joined_summary_{normalized_fsa}')
      nrcan_joined_summary.create_spatial_index()
      _log_layer_summary(nrcan_joined_summary)

    with _workflow_step('extract nrcan restored records', normalized_fsa):
      nrcan_joined_summary.extract_by_expression(
        '"restore_group" = 1',
        output_paths['nrcan_restored'])
      nrcan_restored = ScrubLayer(
        paths.qgis_path,
        output_paths['nrcan_restored'],
        f'nrcan_restored_{normalized_fsa}')
    ...
    with _workflow_step('join nrcan restored with dominant usage id',
      ...
      nrcan_restored_with_usage_id = ScrubLayer(
        paths.qgis_path,
        output_paths['nrcan_restored_with_usage_id'],
        f'nrcan_restored_with_usage_id_{normalized_fsa}')
